...
// Angular Virtual Scroll Repeat v1.0.0-rc7 2015/05/25
//
		...
		return {
			...
			compile: function ($element, $attrs) {
				...
				if ((!window.getComputedStyle || window.getComputedStyle($element[0]).position !== 'absolute') &&
					!$element.hasClass('vs-no-position')
				) {
				...
				return {
					pre: function ($scope, $element, $attrs, $ctrl) {
						...
						var childClone = angular.element(childCloneHtml),
							...
							$scrollParent = $attrs.vsScrollParent ? closestElement.call($element,
								$attrs.vsScrollParent) : $element,
						...
						if (typeof document.documentElement.style.transform !== "undefined") { // browser supports transform css property
							childClone.attr('style',
								prevStyle + '; transform: ' + positioningPropertyTransform + '({{' + offsetCalculationString + '}}px)');
						}
						else if (typeof document.documentElement.style.webkitTransform !== "undefined") { // browser supports -webkit-transform css property
							childClone.attr('style',
								prevStyle + '; -webkit-transform: ' + positioningPropertyTransform + '({{' + offsetCalculationString + '}}px)');
						}
						else {
							childClone.attr('style',
								prevStyle + '; ' + positioningProperty + ': {{' + offsetCalculationString + '}}px');
						}

						$compile(childClone)($scope);
						$element.append(childClone);

						$scope.vsRepeatFillCss = {
							'position': 'relative',
							'min-height': '100%',
							'min-width': '100%'
						};

						if ($attrs.vsRepeatFillCss) {
							$scope.vsRepeatFillCss = JSON.parse($attrs.vsRepeatFillCss);
						}

						$fillElement = angular.element('<div class="vs-repeat-fill-element"></div>')
							.css($scope.vsRepeatFillCss);

						if ($attrs.vsHolder) {
							angular.element($attrs.vsHolder).append($fillElement);
						} else {
						...
						if ($attrs.vsWindowScroll) {
							angular.element(window).on('scroll', function scrollHandler(e) {
						...
						function updateInnerCollection(data) {
							...
							if (digestRequired) {
								...
								// Emit the event
								$scope.$emit('vsRepeatInnerCollectionUpdated',
									$scope.startIndex,
									$scope.endIndex,
									_prevStartIndex,
									_prevEndIndex,
